...
class CompilerToolInput(BaseModel):
    project_path: Optional[str] = Field(None,  # Changed to Optional
...
class SpringBootCompilerTool(BaseTool):
    ...
    _default_spring_code_path_internal: Path  # Internal instance attribute

    def __init__(self, spring_code_path: str, **kwargs: Any):
        super().__init__(**kwargs)  # Call BaseTool's Pydantic init first

        self._default_spring_code_path_internal = Path(spring_code_path).resolve()
        if not self._default_spring_code_path_internal.is_dir():
            logger.error(
                f"SpringBootCompilerTool: Default Spring Boot code path '{self._default_spring_code_path_internal}' is not a valid directory.")
            raise ValueError(
                f"Default Spring Boot code path '{self._default_spring_code_path_internal}' for SpringBootCompilerTool is not a valid directory.")
    ...
    def _compile_project(self, project_path_str: str) -> Dict[str, Any]:
        ...
        if pom_file.exists():
            ...
            command = ["mvn", "-f", str(pom_file), "clean", "package", "-DskipTests"]
            ...
            command = [gradle_cmd_prefix, "-p", str(project_path), "clean", "build", "-x", "test"]
        else:
        ...
        try:
            ...
            if process.returncode == 0:
                ...
                result_message += f"{build_tool_used} compilation/package failed (return code: {process.returncode}).\n"
            ...
            return {"status": "error",
                    "message": f"An exception occurred during {build_tool_used} compilation/package: {e}"}

    def _run(self, project_path: Optional[str] = None) -> str:
        path_to_compile = Path(project_path).resolve() if project_path else self._default_spring_code_path_internal
