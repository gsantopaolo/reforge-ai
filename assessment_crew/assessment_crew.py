...
class AssessmentCrew():
    ...
    def __init__(self,  source_code_path: str):
        # initialize tools with source code path
        self._source_code_path = source_code_path
        self.jdeps_tool = JDepsTool(base_path=self._source_code_path, java_home=os.getenv("JAVA_HOME"))
        self.sonar_tool = SonarTool()
        self.db_tool = DBParserTool(java_helper_jar=os.getenv("JSQL_HELPER_JAR"))
        self.dir_read_tool = DirectoryReadTool(result_as_answer=True, directory=source_code_path)
    ...
    def documentation_agent(self) -> Agent:
        return Agent(
            ...
            llm=llm_client,
            tools=[self.dir_read_tool, self.file_read_tool],
            verbose=True
        )
